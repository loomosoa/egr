...
Однако, есть базовый сценарий, если какое-то или какие-то операнды отрицательные настолько, что f уменьшиться. 
...
Значит отрицательным числом, ведущим "в минус" может быть либо a, либо b, либо c
...
Напоминает матрешку, декоратор.

Нам необязательно понимать концепцию логарифма, чтобы понять, что он делает, если мы видим сразу элементарную геометрию множеств.

И сидя перед нотами, мочь уловить базовую мелодию, услышать ее.

И имея перед собой клавиши пианино ее подобрать, и иметь подсказки, какие клавиши нажать, чтобы ее сыграть.

Это не будет виртуозная игра на фортепиано, но это будет редукция нот оркестра в примитивную (элементарную) мелодию.
