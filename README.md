...
Допустим, мы (программно) вычисляем "операнды", действия, порядок преобразований (множеств).
...
Значит отрицательным числом, ведущим "в минус" может быть либо a, либо b, либо c (в визуализации буквы q, r, s отражают множества, и сами по себе условны)

c = c * c * c

![fxy_qrs1](imgs/fxy_qrs1.png)

В множестве k мы отражаем c, а так же затем подспудно ab, как одно множество k.

![fxy_qrs2](imgs/fxy_qrs2.png)

И на схемке показано, что множества s и к будут одним множеством.
...
И не так важно, если мы, пытаясь понять концепцию, ошибаемся, сколько раз множество B может разрастись.
...
И так же, как нам необязательно понимать концепцию логарифма в привычном объяснении, чтобы понять, что он делает, возможно, необязательно понимать и другие концепции, если они редуцированы в элементарную геометрию корректно?
...
Сразу целиком в итоговую идею того, что делает выражение, или же по этапам, отразив визуально ключевые концепты? Чтобы иметь возможность посмотреть элементарную геометрию, почему итоговый вывод именно такой?
...
И интересный актуальный вопрос в том, какова должна быть логика, спрятанная за методом transform(): возвращающим значения для визуализации.
